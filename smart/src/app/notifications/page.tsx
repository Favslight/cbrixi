...
export default function NotificationsPage() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/notifications?status=${status}`, {
  ...
  const deleteNotification = async (id: string) => {
    ...
    try {
      ...
      // Soft-delete: only remove from UI after successful DELETE
      if (res.ok) {
        setNotifications((prev) => prev.filter((n) => n.id !== id));
      }
    } catch {
      // keep notification visible until delete succeeds
    } finally {
      setDeletingId(null);
    }
  };
      ...
      <div className="max-w-3xl mx-auto pt-28 px-4 pb-20">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <button
            ...
            className="px-4 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-sm font-medium transition-colors border border-white/10 self-start"
          >
            Mark all as read
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white/5 border border-white/10 p-12 rounded-2xl text-center text-white/50">
            ...
            You have no notifications in this filter.
          </div>
        ) : (
